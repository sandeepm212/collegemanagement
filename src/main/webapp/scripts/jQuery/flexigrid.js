...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		p = $.extend({
			 ...
			 onSuccess: false,
			 onError: false,
			 onTrClick: false, // function to be invoked when cliked on the tr
			 editLink: false,
		...
		var g = {
			...
			addData: function (data) { //parse data
				...
				$('.pReload',g.gDiv).removeClass('loading');
				this.loading = false;

				if (!data) 
					{
					$('.pPageStat',g.gDiv).html(p.errormsg);	
					return false;
					}
				...
				if (p.dataType=='xml')
					...
					p.total = data.totalResultsCount;
							 	...
							 	function ()
									{	
										...
										if (p.colModel[idx].name.indexOf(".") != -1) {
											td.innerHTML = eval("row." + p.colModel[idx].name);
										} else if (p.colModel[idx].name == "EDIT_LINK") {
											td.innerHTML = p.editLink();
										} else {
											td.innerHTML = $(row).attr(p.colModel[idx].name);
										}
										$(tr).append(td);
										td = null;
									}
			...
			$('.pcontrol input',g.gDiv).val(p.page);
			$('.pcontrol span',g.gDiv).html(p.pages);
			...
			populate: function () { //get latest data
				...
				this.loading = true;
				if (!p.url && !p.dwrFunction) return false;
				
				$('.pPageStat',g.gDiv).html(p.procmsg);
				
				$('.pReload',g.gDiv).addClass('loading');
			...
			changePage: function (ctype){ //change page
				...
				switch(ctype)
				{
					...
					case 'input': 
							var nv = parseInt($('.pcontrol input',g.gDiv).val());
							if (isNaN(nv)) nv = 1;
							if (nv<1) nv = 1;
							else if (nv > p.pages) nv = p.pages;
							$('.pcontrol input',g.gDiv).val(nv);
							p.newp =nv;
							break;
				}
							...
							.click(
								function (e) 
									{ 
										...
										if (p.onTrClick) p.onTrClick(this);
		...
		// load data
		if ((p.url || p.dwrFunction)&&p.autoload) 
			{
			g.populate();
			}
